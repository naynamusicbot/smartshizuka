...
def cookie_txt_file():
    try:
        ...
        if not txt_files:
            raise FileNotFoundError("⚙️ No .txt files found in the specified folder.")
        ...
        def audio_dl(vid_id):
            try:
                ...
                if os.path.exists(xyz):
                    return xyz
                getAudio = requests.get(f"{BASE_API_URL}/audio/{vid_id}", headers=headers, timeout=60)
                try:
                ...
                if status == 'success':
                    ...
                    print("⚙️ Could not fetch Backend Please contact API provider.")
                    return None
